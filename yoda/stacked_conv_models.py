...
class CustomSqueeze(nn.Module):
    ...
    def reducing_channel(self, inp_channel, kernel=1):
        return nn.Sequential(
            ...
            nn.SiLU(),
            nn.Conv2d(inp_channel, inp_channel - 1, kernel, 1, padding=kernel//2),
        )
...
class LargeKernelNet(nn.Module):
    def __init__(self, channels, kernels=(7, 5)) -> None:
        ...
        self.route_k1x1 = CustomSqueeze(inp_ch=channels, kernel=1)
        self.conv1x1 = nn.Conv2d(len(kernels) + 1, 1, 1, 1, 0)
    
    def forward(self, x):
        route_k1x1_out = self.route_k1x1(x)
        outputs = [route_k1x1_out]
        for k in self.large_kernels:
