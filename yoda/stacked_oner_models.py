...
class SqueezeOneRoute(nn.Module): # out -> channels-1
    def __init__(self, channels, kernels=(3, 5)):
        super().__init__()
        self.conv_1 = nn.Conv2d(channels, channels, 1, 1, 0)
        
        self.kernels = kernels # Except 1x1
        for k in self.kernels:
            setattr(self, f"conv_{k}", ConvBn2d(channels, k))   # Depth-wise
        
        self.conv_skip_act = nn.SiLU()
        self.pw_out = nn.Conv2d(channels, channels-1, 1, 1, 0)
    
    def forward(self, x):
        outp = self.conv_1(x)
        for k in self.kernels:
            outp += getattr(self, f"conv_{k}")(x)
        
        pw_out = self.pw_out(self.conv_skip_act(outp))
        return pw_out


class CustomSqueezeOneRoute(nn.Module):
    # CustomSqueeze from 7ch -> 1ch
    def __init__(self, inp_ch, kernels=(3, 5),) -> None:
        super().__init__()
        self.r1 = SqueezeOneRoute(channels=inp_ch, kernels=kernels) # inp_ch -1
        self.r2 = SqueezeOneRoute(channels=inp_ch - 1, kernels=kernels)
        self.r3 = SqueezeOneRoute(channels=inp_ch - 2, kernels=kernels)
        self.r4 = SqueezeOneRoute(channels=inp_ch - 3, kernels=kernels)
        self.r5 = SqueezeOneRoute(channels=inp_ch - 4, kernels=kernels)
        self.r6 = SqueezeOneRoute(channels=inp_ch - 5, kernels=kernels)
...
class LargeKernelNet(nn.Module):
    def __init__(self, channels, kernels=(3, 5)) -> None:
        super().__init__()
        self.one_route = CustomSqueezeOneRoute(channels, kernels=kernels)
