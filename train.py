...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze, fe_models = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg, \
        opt.resume, opt.noval, opt.nosave, opt.workers, opt.freeze, opt.fe_models
            ...
            if f.exists():
                ...
                if f is best:
                    ...
                    results, _, _ = validate.run(
                        ...
                        callbacks=callbacks,
                        compute_loss=compute_loss,
                        fe_models=fe_models)  # val best model with plots
                    if is_coco:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/data_npy.yaml', help='dataset.yaml path')
    ...
    # Select Feature Extractor
    parser.add_argument('--fe_models', type=str, default='FE_Add', help='Name of Feature Extractor model')
