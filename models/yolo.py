...
class Stack(nn.Module):
    def __init__(self,):
        ...
        with open('models/stats.yaml', 'r') as file:
...
class LargeKernelNet(nn.Module):
    def __init__(self, channels, kernels=(1, 3)) -> None:
        super().__init__()
...
class BaseModel(nn.Module):
    ...
    def forward(self, x, profile=False, visualize=False):
        return self._forward_once(x, profile, visualize, return_latent=True)  # single-scale inference, train

    def _forward_once(self, x, profile=False, visualize=False, return_latent=True):
        y, dt = [], []  # outputs
        x, stacked_im = self.YODA(x)
        latent_im = torch.clone(x)
        for m in self.model:
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        # Loading YODA net
        kernel_size = tuple(self.yaml['kernel_size'])
        self.YODA = YODA(channels=7, kernels=kernel_size)
        ...
        if isinstance(m, (Detect, Segment)):
            ...
            forward = lambda x: self.forward(x, return_latent=False)[0] if isinstance(m, Segment) else self.forward(x, return_latent=False)
            m.stride = torch.tensor([s / x.shape[-2] for x in forward(torch.zeros(1, 1, s, s))])  # forward
    ...
    def forward(self, x, augment=False, profile=False, visualize=False, return_latent=True):
        if augment:
            return self._forward_augment(x, return_latent=return_latent)  # augmented inference, None
        return self._forward_once(x, profile, visualize, return_latent=return_latent)  # single-scale inference, train

    def _forward_augment(self, x, return_latent=True):
        ...
        for si, fi in zip(s, f):
            ...
            yi, (latent_im, stacked_im) = self._forward_once(xi, return_laten=return_latent)
            yi = yi[0]
            # yi = self._forward_once(xi, return_latent=return_latent)[0][0]  # forward
